...
def linearrgb_to_srgb(c):
    if c < 0.0031308:
        ...
        return (pow(c, 0.41666) * 1.055) - 0.055

def linearrgb_to_srgb_u16(c):
    return 65535 * linearrgb_to_srgb(c / 65535) + 0.5 # bias for rounding
...
class glTF2ExportUserExtension:
    ...
    def gather_attribute_change(self, attribute, data, is_normalized_byte_color, export_settings):
        if self.apply_mesh_srgb_fixup and attribute.startswith("COLOR_") and is_normalized_byte_color is True:
            print(str.format("Linear -> sRGB fixup on color channel {0}, dtype {1}", attribute, data['data'].dtype))
            if data['data'].dtype == numpy.uint16:
                for color in data['data']:
                    color[0] = linearrgb_to_srgb_u16(color[0])
                    color[1] = linearrgb_to_srgb_u16(color[1])
                    color[2] = linearrgb_to_srgb_u16(color[2])
            elif data['data'].dtype == numpy.float32:
                for color in data['data']:
                    color[0] = linearrgb_to_srgb(color[0])
                    color[1] = linearrgb_to_srgb(color[1])
                    color[2] = linearrgb_to_srgb(color[2])
            else:
